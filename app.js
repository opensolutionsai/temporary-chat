...
function setupEventListeners() {
    ...
    document.getElementById('btn-copy-link').addEventListener('click', () => {
        const url = `${window.location.origin}${window.location.pathname}?room=${appState.roomId}`;
        navigator.clipboard.writeText(url).then(() => alert('Public Link copied (Users will need password)'));
    });
    ...
    document.getElementById('btn-copy-link-pass').addEventListener('click', () => {
        ...
        const url = `${window.location.origin}${window.location.pathname}?room=${appState.roomId}&pass=${encodeURIComponent(appState.rawPassword)}`;
...
async function joinRoom(roomId, nickname, password) {
    // 0. Check Ban Status
...
async function destroyRoom() {
    ...
    const { error } = await supabaseClient
        .from('rooms')
        .delete()
        .eq('id', appState.roomId);

    if (error) {
        alert("Failed to destroy room: " + error.message);
    } else {
        // Client will react to DELETE event, but we can also leave immediately
        // leaveRoom(); // Optional, let the event handle it for consistency
    }
}
...
function renderUserList(users) {
    ...
    users.forEach(user => {
        ...
        let badges = '';
        if (user.is_admin) badges += `<span class="user-badge admin">Host</span>`;
        if (user.nickname === appState.nickname) badges += `<span class="user-badge">You</span>`;

        let action = '';
        // Only show kick button if: I am admin, Target is not me
        if (appState.isCreator && user.nickname !== appState.nickname) {
            action = `<button class="btn-kick" onclick="kickUser('${user.id}')">Kick</button>`;
        }
